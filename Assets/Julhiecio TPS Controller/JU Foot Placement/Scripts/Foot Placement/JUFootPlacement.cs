    ...
    public class JUFootPlacement : MonoBehaviour
    {
        ...
        private void OnAnimatorIK(int layerIndex)
        {
            if (Vector3.Angle(transform.up, Vector3.up) > 30 && EnableFootPlacement)
            {
                ...
                //Debug.LogWarning("JU Foot Placement is not supported at certain angles, it is convenient to turn it off when the character's up axis is in a different direction from the world up axis.");
            }
        ...
        private void OnDrawGizmos()
        {
            ...
            {
                GUIStyle TextStyle = JUTPSEditor.CustomEditorStyles.Title();
