    ...
    public class FieldOfView
    {
        ...
        private void Scan()
        {
            ...
            for (int i = 0; i < foundCount; i++)
            {
                ...
                if (!IsAllowedTarget(collider.transform))
                {
                    _detections[i] = null;
                    continue;
                }
            ...
            for (int i = 0; i < foundCount; i++)
            {
                ...
                if (distance < minDistance)
                {
                    ...
                    if (_detections[i].TryGetComponent(out IHealth health))
                    {
                        if (health.IsDead)
                        {
                            continue;
        ...
        public bool IsOnView(Collider otherCollider)
        {
            ...
            if (!IsAllowedTarget(otherCollider.transform))
            {
                return false;
            }
        ...
        /// <summary>
        /// Return true only if the object contains the correct tag and layer.
        /// </summary>
        /// <param name="target">The object to check.</param>
        /// <returns></returns>
        public bool IsAllowedTarget(Transform target)
        {
            if (!target)
            {
                return false;
            ...
            // Does not have correct layer.
            if ((TargetsLayer & (1 << target.gameObject.layer)) == 0)
            {
                return false;
            }
            ...
            if (TargetTags.Length > 0)
            {
                ...
                for (int x = 0; x < TargetTags.Length; x++)
                {
                    if (target.CompareTag(TargetTags[x]))
                    {
                        hasTag = true;
                        break;
                    }
