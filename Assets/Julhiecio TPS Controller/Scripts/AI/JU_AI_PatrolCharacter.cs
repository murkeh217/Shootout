...
namespace JU.CharacterSystem.AI
{
    ...
    {
        /// <summary>
        /// What AI will do when hear something?
        /// </summary>
        public enum OnHearSoundBehaviors
        {
            /// <summary>
            /// Go to the possible target position and search for it.
            /// </summary>
            SearchTarget,

            /// <summary>
            /// Go to the target possition and attack immediately instead of search first.
            /// </summary>
            AttackTarget
        }
        ...
        /// What AI will do when hear something?
        /// </summary>
        public OnHearSoundBehaviors OnHearSoundBehavior;
        ...
        /// </summary>
        public IHealth CurrentTargetHealth
        {
            get
            {
                if (_targetHealth == null && _target)
                {
                    _targetHealth = CurrentTarget.GetComponent<IHealth>();

                    if (_targetHealth == null && _target.transform.parent)
                    {
                        _targetHealth = _target.transform.parent.GetComponent<IHealth>();
                    }
                }
        ...
        protected override void Start()
        {
            ...
            if (JU_Ai.ClosestToNavMesh(_spawnPosition, out Vector3 spawnPointOnNavmesh))
            {
        ...
        protected override void Update()
        {
            ...
            if ((CurrentState == PatrolStates.Attacking && !CurrentTarget) || (CurrentTargetHealth != null && CurrentTargetHealth.IsDead))
            {
                CurrentTarget = null;
        ...
        private void OnHearSomething(Vector3 position, GameObject source)
        {
            ...
            Collider sourceCollider = source ? source.GetComponent<Collider>() : null;
            if (OnHearSoundBehavior == OnHearSoundBehaviors.AttackTarget && sourceCollider && FieldOfView.IsAllowedTarget(source.transform))
            {
                _loseTargetTimer = 0;
                CurrentTarget = sourceCollider;
                CurrentState = PatrolStates.Attacking;
            }
            else
            {
