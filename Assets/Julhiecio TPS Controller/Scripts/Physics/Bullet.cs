    ...
    public class Bullet : MonoBehaviour
    {
        ...
        void OnCollisionEnter(Collision col)
        {
            if (col.gameObject.tag != "Bullet")
            {
                ...
                IHealth.DamageInfo damageInfo = new IHealth.DamageInfo
                {
                    ...
                    HitOriginPosition = _startPosition,
                    DamageOwner = Owner
                };
                ...
                if (col.gameObject.layer == 15 || col.gameObject.layer == 9)
                {
                    ...
                    {
                        Physics.IgnoreCollision(col.collider, _selfCollider);
                        return;
                    }
                ...
                if (GetOwner() != null && Owner.CompareTag("Player"))
                {
                    if (col.gameObject.layer == 15 || col.gameObject.layer == 9)
                    {
                        ...
                        {
                            if (col.gameObject.GetComponentInParent<IHealth>().IsDead == false)
                            {
        ...
        public void Ignore(Collider[] collider)
        {
            foreach (Collider col in collider)
            {
                if (col)
                {
                    Physics.IgnoreCollision(_selfCollider, col);
                }
            }
        }
